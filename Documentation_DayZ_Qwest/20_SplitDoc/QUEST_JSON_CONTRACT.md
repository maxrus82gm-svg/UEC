...
- каждый квест должен закрываться через `Reward`;
- у каждого квеста должен быть финальный `rewardTriggerIds[]` и соответствующий `triggerActions[]` с `actionType: "reward"`;
- это правило действует даже если квест берётся и закрывается у одного и того же NPC / trigger;
- `Offer` только выдаёт квест, а `Completion` только промежуточный этап; ни `Offer`, ни `Completion` не должны считаться финальным закрытием квеста.
